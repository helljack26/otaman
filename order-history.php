...
<head>
    ...
    <title>Історія замовлень | Otaman.net</title>
        ...
        <div class="container" style="display: flex; flex-direction: column-reverse; margin-bottom: 60px;">
                ...
                while($payment_order = mysqli_fetch_array($payment_order_select)){  
                    ...
                    if ($order_paymentMethod == 'Онлайн оплата карткою (LiqPay)') {
                        ...
                        $data['form'] = $liqpay->cnb_form(array(
                            ...
                            'language'		=> 'ua',
                            'type'			=> 'donate',
                            'result_url'	=> 'https://otaman.net/thanks.php',
                            'server_url'	=> 'https://otaman.net/order-history.php',
                            'action' 		=> 'pay',
                        ));
                        #liq pay status  
            ...
            <table class="table table-bordered">
                <tbody>
                    ...
                    <?php
			            $can_pay_sql = mysqli_query($con,"SELECT * from orders where userId='$_SESSION[id]' and collapse_order_id='$order_id'");
                        while($row_can = mysqli_fetch_array($can_pay_sql)){  
                            $can_pay = $row_can['can_pay'];
                        }?>
                    <div class="main_border">
                        <div class="row" style="text-align: center;vertical-align: middle;font-size:22px;">
                            ...
                            <? if($can_pay == '1' || $can_pay == '2'){ 
                                    if ($order_paymentMethod == 'Онлайн оплата карткою (LiqPay)'){
                                        echo($pay_result);
                                    } elseif ($order_paymentMethod == 'Готівкою або картою при отриманні') {?>
