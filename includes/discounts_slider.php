<?
require_once 'includes/config_vikar.php';
?>
<!-- Similar products -->
<div class="discount_container wow fadeInUp">
    <div class="discount_container_header">
        <h3 class="discount_container_header_text">Супутні товари</h3>
    </div>

    <div id="similar_slider" class="discount_slider">
        <?php
            $promo = $dbh->prepare('SELECT * from products  where sub_сategory= ? and category=? ORDER BY product_availability DESC LIMIT 10');
            ...
            while ($row2 = $promo->fetch(PDO::FETCH_LAZY)): 	
                ...
                $c_photo2r		= $row2['productImage2'];
                $c_photo3r		= $row2['productImage3'];
                $c_photo4r		= $row2['productImage4'];
                $c_photo5r		= $row2['productImage5'];
                $c_photo6r		= $row2['productImage6'];
            ...
            <div class="discount_slider_item">
                <div class="<?php if($row2['product_availability'] == 0 || $row2['product_count'] ==0){
                    ...
                    <div class="discount_slider_item_image">
                        <a href="product-details.php?pid=<?php echo $c_coder;?>">
                            <img src="<?if($row2['productImage1']=='') {echo'categoryImage/no_foto.png'; }else{echo '../images/'.$row2['productImage1'];}?>"
                                data-echo="<?if($row2['productImage1']=='') {echo'categoryImage/no_foto.png'; }else{echo '../images/'.$row2['productImage1'];}?>"
                                height="210px" alt="">
                        </a>
                    </div>
                    ...
                    <div class="discount_slider_item_info">
                        <!-- Product rating -->
                        <?php 
                        $rt=mysqli_query($con,"SELECT * from productreviews where productId='$c_coder'");
                        $num=mysqli_num_rows($rt);
                        $averageRating = 0;
                        while($rowRating = mysqli_fetch_array($rt)){
                        ...
                        if ($num == 0) :?>
                            <div class="reviews">
                                <a href="product-details.php?pid=<?php echo htmlentities($c_coder);?>#review">
                        ...
                        <h3 class="discount_slider_item_info_name">
                            <a href="product-details.php?pid=<?php echo htmlentities($row2['c_code']);?>">
                ...
                <div class="discount_slider_item_footer">
                    ...
                    <button type="button" data-prices="<?php echo htmlentities($row2['product_price']);?>"
                        ...
                        data-code="<?php echo $row2['c_code']?>" class="discount_slider_item_footer_basket
                        <? if (isExistInBasket($row2['c_code']) != 'Купити') {
                                echo('discount_slider_item_footer_basket_active');
                            }?>">
                        <? echo(isExistInBasket($row2['c_code']));?>
                    </button>
                    ...
                    <!-- Wishlist button -->
                    <?
                    $isInWishListRecommend = mysqli_query($con,"SELECT productId from wishlist where productId='$row2[c_code]' and userid='$_SESSION[id]'");
